...
public class PermisoFuncional implements CodigoDescripcion<String, String> {
    ...
    public static Optional<PermisoFuncional[]> buscarCandidatos(Permiso[] permisos, TipoPermiso tipo) {
        return buscarCandidatos(permisos, tipo.getValor() % 100);
    }

    public static Optional<PermisoFuncional[]> buscarCandidatos(Permiso[] permisos, int tipo) {
        return buscarCandidatos(permisos, RecursoEcomarket.valueOf(tipo));
    }

    public static Optional<PermisoFuncional[]> buscarCandidatos(Permiso[] permisos, RecursoEcomarket recurso) {
        return buscarCandidatos(
                Arrays.stream(permisos).map(PermisoFuncional::new).toArray(PermisoFuncional[]::new),
                recurso
        );
    }

    public static Optional<PermisoFuncional[]> buscarCandidatos(PermisoFuncional[] permisos, RecursoEcomarket recurso) {
        return buscarCandidatos(permisos, recurso.getCodigo());
    }

    public static Optional<PermisoFuncional[]> buscarCandidatos(PermisoFuncional[] permisos, int recurso) {
        return Arrays.stream(permisos)
                     .filter(p -> equalsRecurso(p.getTipo().getValor(), recurso))
                     .toArray(PermisoFuncional[]::new)
                .length > 0 ? Optional.of(permisos) : Optional.empty();
    }
